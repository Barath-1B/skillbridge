...
export default function MbtiQuiz() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await api.get('/profile/mbti/questions');
        if (mounted) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await api.post('/profile/mbti', { answers: answerArray });
      await refreshUser();
      navigate('/profile');
    } catch (err) {
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs uppercase tracking-wider font-semibold text-teal-700 dark:text-teal-300">
            Personality · MBTI · Q{Math.min(current + 1, Math.max(total, 1))} of {total || '—'}
          </p>
